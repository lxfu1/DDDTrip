...
export const getWebGLDebugUtils = ()=>  {
   ...
   *
   * Example:
   *   let str = WebGLDebugUtil.glEnumToString(ctx.getError());
   *
  ...
  const makeDebugContext = (ctx, opt_onErrorFunc?: any) => {
    init(ctx);
    opt_onErrorFunc =
      opt_onErrorFunc ||
  ...
  const  makeLostContextSimulatingContext = (ctx) => {
    ...
    function makeLostContextWrapper(ctx, functionName) {
      ...
      return function () {
        ...
        if (!contextLost_) {
          ...
          let result = f.apply(ctx, arguments);
          return result;
        }
      };
    }
    ...
    let functionsThatShouldReturnNull = [
      "getActiveAttrib",
      "getActiveUniform",
      "getBufferParameter",
  ...
  return {
     ...
     * Example:
     *   WebGLDebugUtil.init(ctx);
     *   let str = WebGLDebugUtil.glFunctionArgToString('bindTexture', 0, gl.TEXTURE_2D);
     *
